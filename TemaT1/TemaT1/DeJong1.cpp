...
DeJong1::DeJong1()
{}
double DeJong1::DeJong1Functie() 
{
	double s;
	s = 0;
	for (int i = 0; i < n; ++i)
		s += (x_valori[i] * x_valori[i]);
	return s;
}

DeJong1::DeJong1(int dim)
{n = dim;
a = -5.1200;
b = 5.1199;
d = 5;
...
void DeJong1::Decodificare(bool sir[])
...
void DeJong1::HillClimbingFirst()
{
	bool local;
...
for (int y = 0; y < n; y++)
	cout << x_valori[y] << ' ';
cout << endl;*/ //afisam valorile obtinute
		if (iteratii == 0)
		...
		Decodificare(vc);
		potentialmin = DeJong1Functie();
		Decodificare(best);
		if (DeJong1Functie() > potentialmin)
		{
			copy(vc, vc + n * nrbiti, best);
			Decodificare(best);
			//cout << DeJong1Functie() << endl;
		}
	}
	Decodificare(best);
}
void DeJong1::HillClimbingBest()
{
	bool local;
	double min,potentialmin;
	//srand(time(NULL));
...
void DeJong1::SimulatedAnnealing()
{
	...
	{nrveciniparcursi = 0;
		...
		do
		{
			...
			if (vn[poz] == 1)
				...
				vn[poz] = 1;
			Decodificare(vc);
			Value = DeJong1Functie();
			min = Value;
			Decodificare(vn);
			Neighbor = DeJong1Functie();
			if (Neighbor < min)
			{local = 1;
			copy(vn, vn + n*nrbiti, vc);
			}
